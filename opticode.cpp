...
int main() {
    ...
    // ==== primitive segments + seg_groups (for next-use computation) ====
    vector<int> b;
    b.reserve(2 * N + 4);
    ...
    auto interval_next_use = [&](int Lx, int Rx, int cur_gi)->int {
        ...
        int earliest = INT_MAX; // earliest future group that will use any piece
        for (int sid = li; sid < ri; ++sid) {
            ...
            if (it != gv.end()) earliest = min(earliest, *it);
                ...
                // this primitive has no future use -> we can mark as INF by setting earliest = INT_MIN marker
                // but we treat "no future use" as very large next-use; return INT_MAX to represent never-used.
                return INT_MAX;
            }
        }
    ...
    for (size_t gi = 0; gi < groups.size(); ++gi) {
        ...
        if (need_bytes + cur_loaded > M) {
            ...
            // select freeable segments by Belady-like rule:
            // 1) segments with no future use (next_use == INF) first;
            // 2) otherwise segments with farthest next use (larger next_use) first.
            ...
            sort(cands.begin(), cands.end(), [](const Cand &A, const Cand &B){
                // INT_MAX (never used) should come first; otherwise sort by next_use descending (farthest first)
        ...
        if (need_bytes + cur_loaded > M) {
            ...
            for (auto seg : snap) {
                ...
                if (!ok) continue;
                // compute next use for this whole segment
                int nu = interval_next_use(seg.first, seg.second, (int)gi);
                // prefer segments with nu == INF or large nu; push others to later by skipping now
                if (nu != INT_MAX) {
                    // postpone this segment by pushing to end of snap to try other segments first
                    snap.push_back(seg);
                    continue;
                }
